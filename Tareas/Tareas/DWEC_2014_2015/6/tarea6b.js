...
function iniciar(){	
	...
	document.getElementById("sumar_rojo").addEventListener("click", sumar, false);
	document.getElementById("sumar_blanco").addEventListener("click", sumar, false);
...
function pintarCelda(evt){
	var colorActivo = document.getElementById("color").innerHTML.split(":")[1].trim();
	if (colorActivo =='rojo') {
...
function activarColor() {
	document.getElementById("color").innerHTML = 'Color activo: ' + this.id;
}
...
function sumar() {
	...
	// pintar suma
	color = color.replace(/o$/, 'a')
	document.getElementById("suma").innerHTML = 'Las ' + color + 's suman ' + suma;
}