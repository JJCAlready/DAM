...
abstract class Modelo {
	...
	public function __construct($atributos=array()) {
		...
		if(is_null($this->idAutoIncrementable)) {
			// si no se asigna un valor al atributo $idAutoIncrementable asumimos que será true
			$this->idAutoIncrementable =  true;
		}
	...
	public function create() {
		...
		try {	
			$todoOK=$pdo->exec($strQuery);
			// si realiza correctamente la inserción, recuperar el id del registro si fuese necesario
			if($this->idAutoIncrementable) {
	 ...
	 * En caso de modificarse el campo id, se deberá pasar como parámetros el
	 * valor de dicho campo antes de la modificación
	 * @return true si la operación se realizó correctamente, false en caso contrario
	 */
	public function update($codAnt=null) {
		if (is_null($codAnt)) {
			$codAnt = $this->atributos["$this->id"];
		} 
		$todoOK = true;
		$pdo = $this->dbConect();
		// construimos el string para preparar la consulta
		...
		// preparamos la parte where según sea el tipo de columna del campo id
		$tipoColumna = $this->getTipoColumna($this->id);
		switch ($tipoColumna) {
				case 'date':
					$strWhere = "$this->id='".formatFechaBD($codAnt)."'";
					break;			
				case 'varchar':
				case 'char':
					$strWhere = "$this->id='".$codAnt."'";
					break;
				case 'int':	
				case 'float':
				default:
		...
		// creamos la consulta
		$strQuery = "UPDATE $this->tabla SET $strSet WHERE $strWhere";
		// ejecutamos la consulta procesando los posibles errores como
		// excepciones
		$this->errores = null;
		$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	...
	public function delete() {
		$todoOK = true;
		$pdo = $this->dbConect();
		// preparamos la parte where de la consulta según sea el tipo de columna del campo id
		$tipoColumna = $this->getTipoColumna($this->id);
		switch ($tipoColumna) {
				case 'date':
					$strWhere = "$this->id='".formatFechaBD($this->atributos["$this->id"])."'";
					break;			
				case 'varchar':
				case 'char':
					$strWhere = "$this->id='".$this->atributos["$this->id"]."'";
					break;
				case 'int':	
				case 'float':
				default:
		...
		// creamos la consulta
		$strQuery = "DELETE FROM $this->tabla WHERE $strWhere";
		// ejecutamos la consulta procesando los posibles errores como
		// excepciones
		$this->errores = null;
		$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	...
	/**
	 * Retorna un array con todos los registros de la tabla 
	 * @return array
	 */
	public function all() {
		// crearemos un objeto de la clase para la que es llamada la función 
		...
		if(!is_null($pdo)) {
			...
			try {	
				// Ejecutamos una sentencia preparada pasando un array de valores
				// evitando inyecciones SQL maliciosas
				return $pdo->query("SELECT * FROM $tabla");
